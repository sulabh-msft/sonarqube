...
public class SqaleNewMeasuresVisitorTest {
  ...
  private static final int UT_FILE_REF = 11112;
  ...
  @Test
  public void unit_test_files_have_no_new_debt_ratio() {
    treeRootHolder.setRoot(
      builder(PROJECT, ROOT_REF)
        .addChildren(
          builder(FILE, UT_FILE_REF).setFileAttributes(new FileAttributes(true, null)).build()
        )
        .build()
      );
    ...
    assertNoNewDebtRatioMeasure(UT_FILE_REF);
    assertNewDebtRatioValues(ROOT_REF, 0, 0);
  }

  @Test
  public void file_have_no_new_debt_ratio_if_scm_dates_are_all_before_snapshot_dates() {
    treeRootHolder.setRoot(
      builder(PROJECT, ROOT_REF)
        .addChildren(
    ...
    measureRepository.addRawMeasure(LANGUAGE_1_FILE_REF, NCLOC_DATA_KEY, createNclocDataMeasure(3, 4, 5));
    reportReader.putChangesets(createChangeset(LANGUAGE_1_FILE_REF, PERIOD_2_SNAPSHOT_DATE - 100, 5));
  ...
  public void file_has_0_new_debt_ratio_if_all_scm_dates_are_before_snapshot_dates() {
    ...
    reportReader.putChangesets(createChangeset(LANGUAGE_1_FILE_REF, PERIOD_2_SNAPSHOT_DATE - 100, 4));
  ...
  private void setupOneFileAloneInAProject(int newDebtPeriod2, int newDebtPeriod4, Flag isUnitTest, Flag withNclocLines, Flag withChangeSets) {
    ...
    checkArgument(withNclocLines == Flag.WITH_NCLOC || withNclocLines == Flag.NO_NCLOC);
    ...
    if (withChangeSets == Flag.WITH_CHANGESET) {
      ...
      reportReader.putChangesets(createChangeset(LANGUAGE_1_FILE_REF, PERIOD_2_SNAPSHOT_DATE - 100, 2, PERIOD_2_SNAPSHOT_DATE + 100, 2));
    }
  }

  private enum Flag {
    UT_FILE, SRC_FILE, NO_CHANGESET, WITH_CHANGESET, WITH_NCLOC, NO_NCLOC
  }
   ...
   */
  private static BatchReport.Changesets createChangeset(int componentRef, long scmDate, int lines) {
   ...
   */
  private static BatchReport.Changesets createChangeset(int componentRef, long scmDate, int lineCount, long otherScmDate, int otherLineCount) {
    BatchReport.Changesets.Builder builder = BatchReport.Changesets.newBuilder()
      .setComponentRef(componentRef);
